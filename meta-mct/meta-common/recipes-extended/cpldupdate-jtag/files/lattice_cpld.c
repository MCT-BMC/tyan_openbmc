...
static cpld_device_t device_list[] = {
    ...
    },
    {
        ...
        .dev_id = 0x012bb043,
        .page_bits = 128,
        .num_of_cfg_pages = 3198,
        .num_of_ufm_pages = 640,
    }
};

static jtag_object_t *global_jtag_object = NULL;

jtag_object_t *jtag_hardware_mode_init(const char *dev_name, int freqency)
{
    jtag_object_t *temp_object = jtag_init(dev_name, JTAG_XFER_HW_MODE, freqency);
    if(NULL == temp_object){
...
int program_configuration(int bytes_per_page, int num_of_pages,
                          int process, const char *version_id, progress_func_t progress)
{
    int rc = 0;
    int row;
    int i;
    ...
    for (row = 0 ; row < used_pages; row++){
        ...
        if (page_buffer[0] != 0){
            ...
            if(NULL != progress){
                progress((row*100)/used_pages,process,version_id);
            }
        }
    }
