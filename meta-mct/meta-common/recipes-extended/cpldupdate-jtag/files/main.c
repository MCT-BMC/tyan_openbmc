...
typedef struct {
    ...
    char version_id[100];         /* version id */
...
static const char short_options [] = "dhmeRp:v:f:o:P:";
...
static void update_flash_progress(int pecent, int progress, const char *version_id)
{
    if(progress)
    {
        FILE * fp;
...
static void print_progress_bar(int pecent, int progress, const char *version_id)
{
    int i = 0;
    ...
    //Using 30 as the basic updating progress
    update_flash_progress(((pecent/2)+30),progress,version_id);
...
int cpld_init(cpld_t *pcpld)
{
    ...
    pcpld->pjtag_object = jtag_hardware_mode_init("/dev/jtag0",pcpld->freqency);
    if(NULL == pcpld->pjtag_object) {
        return -1;
    }
...
int cpld_program(cpld_t *pcpld, const char *file_name)
{
    ...
    rc = program_configuration(pcpld->pcpld_device->page_bits/BITS_OF_ONE_BYTE, 
         ...
         pcpld->progress, pcpld->version_id,
         print_progress_bar);
    if(rc < 0){
...
end_of_func:
    if(rc == 0) print_progress_bar(100,pcpld->progress,pcpld->version_id);
    return rc;
}
...
int cpld_verify(cpld_t *pcpld, const char *file_name)
{
    ...
    rc = verify_configuration(pcpld->pcpld_device->page_bits/BITS_OF_ONE_BYTE, 
         pcpld->pcpld_device->num_of_cfg_pages);
    update_flash_progress(90,pcpld->progress,pcpld->version_id);
    if(rc < 0){
...
int main(int argc, char *argv[]){
    ...
    cpld.freqency = 0; /* Getting default freqency from JTAG driver*/
